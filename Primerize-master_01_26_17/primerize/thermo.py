...
def calc_Tm(sequence, DNA_conc=0.2e-6, monovalent_conc=0.1, divalent_conc=0.0015): #NL changed params to match precalc
...
"NL Addition: generate tm matrix for multiple sequences"

def _precalculate_Tm_multi(seq1, seq2, min_length=15, max_length=60, m=6):
    seq1 = seq1.upper()
    seq2 = seq2.upper()
    ...
    #create lookup table for TM values
    #when there are multiple distinct TM scores for a position, take minimum
    tm_precalc = numpy.zeros((N_BP,N_BP),dtype='float') - 273

    for i in xrange(N_BP):        
         for j in xrange(i+2,min(i+max_length,N_BP+1)):
            ...
            #initialize list to store tm scores
            tm_scores = []
            ...
            if (len(starts) == 1 and len(keep_indices==1)):
                ...
                for n in xrange(len(s_rep)):
                    ...
                    collapse = "".join(list(chain(*strings)))
