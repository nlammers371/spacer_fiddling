...
def _check_misprime_multi(sequence1, sequence2, m=6, l=20):
    ...
    return (num_match_forward, num_match_reverse, best_match_forward, best_match_reverse, misprime_score_forward, misprime_score_reverse, swap_track)


def _check_misprime_multi_test(sequence1, sequence2, m=6, l=20):
        ...
        for j in xrange(1,N_BP+1):
            ...
            for k in xrange(len(index_array[1,])):
                ...
                if ((index_array[0,k] < j) and (index_array[1,k] > i)):
                    ...
                    starts1.append(index_array[0,k])
                    stops1.append(index_array[1,k])
        
                    if (index_array[2,k] > 0):
                        #swap_trk.append(ct) 
                        swap.append((index_array[2,k]))
                    else:
                        #keep_trk.append(ct)
                        keep.append((-index_array[2,k]))
                    ct += 1
            old.append([keep,swap])
           #filter for relevant segments
            ...
            new.append([keep_indices + ind_start, swap_indices + ind_start])
            ...
            #otherwise we must enumrate all possible combos         
                ...
                for n in xrange(len(s_rep)):
                    ...
                    for o in xrange(len(bin)):
                        segment.append((swap_indices[o] + ind_start)*(int(bin[o])-(int(bin[o])==0))) # <0 if cons, >0 if swap
    ...
    return (num_match_forward, num_match_reverse, best_match_forward, best_match_reverse, misprime_score_forward, misprime_score_reverse, segmentID)
